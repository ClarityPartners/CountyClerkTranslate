<?php
$text_content = variable_get('text_total_ev', '');
?>
...
<script type="text/javascript">
	...
	var inputViewModelSettings = {
		...
		locationOptionsUrl: "/api/Election/GetEarlyVotingLocationOptions",
		baseUrl: "/api/SpecialVoting/GetEarlyVotingLocationTotal",
		electionCode: "0",
		locationId: "",
		votedOn: "",
		mapToVotedOn: function (result) {
			if (result && result.EarlyOpenDate && result.EarlyCloseDate) {
				var start = new Date(result.EarlyOpenDate());
				var future = new Date(result.EarlyCloseDate());
...
<h3 data-bind="css: { 'text-muted' : ((LocationVotingTotals.refreshing() && !LocationVotingTotals.loaded()) || LocationVotingTotals.refreshing()) }" style="line-height: 36px;">
		<span class="h2">
			<strong>
			<span style="display:block">Early Voting Totals</span>
...
<div class="text-center evtotals-pager" style="display: none;" data-bind="visible: (loaded() && item().length > 0)">
